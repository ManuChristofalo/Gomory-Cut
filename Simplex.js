...
function salvaValor(){
    ...
    while((resultadoSimplex===0  || flagInteiro===0) && count<10){
        count++;
        ...
        if(count===10){
            ...
            resultadoSimplex=pivotamento(); //Cálculo do simplex
            
            console.log("SIMPLEX INICIAL - Iteracao base");
            ...
            flagInteiro=1;
            if((matrizOperacao[numRestr+1][numVariaveis+(numRestr*2)+1]%1).toFixed(5) !==0 ) flagInteiro=0;
        }

        else{
            //Corte de gomory entra aqui
            var linhaNaoInteira = verificaInteiro();
            criaNovaRestricao(linhaNaoInteira);

            resultadoSimplex=pivotamento();
            

            console.log("SIMPLEX GOMORY - Iteracao " + (count-1));
            for(let linha=0; linha<numRestr+2; linha++){
                var array=[];
                for(let coluna=0; coluna<numVariaveis+(numRestr*2)+2; coluna++){
            ...
            let conta=(Math.abs((matrizOperacao[numRestr+1][numVariaveis+(numRestr*2)+1]).toFixed(5))-
            (Math.abs((matrizOperacao[numRestr+1][numVariaveis+(numRestr*2)+1]).toFixed(5))-(matrizOperacao[numRestr+1][numVariaveis+(numRestr*2)+1])%1))%1;
            
            conta=Math.abs(conta);

            let prox;
            if(Math.abs(1-conta)<=Math.abs(0-conta)) prox=Math.abs(1-conta);
            else prox=Math.abs(0-conta)
            console.log("\nProximidade de z com um numero inteiro: " + prox);
            
            if((1-conta<=0.00001 && 1-conta>=-0.00001) || 0+conta<=0.00001 && 0+conta>=-0.00001){
                flagInteiro=1;
                console.log("Logo, podemos considerar Z um inteiro");
            }

            if(flagInteiro){
                for(let i=1; i<=numVariaveis; i++){
                    for(let j=0; j<=numInicialRestr; j++){
                        if(vetorBase[j]===i){
                            if(j===0){
                                if((matrizPadrao[1][numVariaveis+(2*numRestr)+1]%1)!==0){
                                    flagInteiro=0;
                                    console.log("\nMas existe uma variavel NAO INTEIRA: X" + j);
                                }
                            }
                            else{
                                if((matrizPadrao[j][numVariaveis+(2*numRestr)+1]%1)!==0){
                                    flagInteiro=0;
                                    console.log("\nMas existe uma variavel NAO INTEIRA: X" + j);
                                }
                            }
            ...
            if(flagInteiro){
                console.log("\nTodas as variaveis iniciais tambem sao inteiras :)")
                resultadoSimplex=1;
                break;
            }
        }
    ...
    if(resultadoSimplex===-1){
        ...
        console.log("\n\nSimplex impossível (sem solução inteira)");
        span.textContent = "Simplex impossível (sem solução inteira)";
        divResultado.appendChild(span);
    }
    else if(resultadoSimplex===3){
        console.log("\n\bSolução ilimitada/infinita");
        span.textContent = "Solução ilimitada/infinita";
        divResultado.appendChild(span);
    }
    else if(resultadoSimplex===4){
        console.log("\n\nMúltiplas soluções");
        span.textContent = "Múltiplas soluções inteira";
        divResultado.appendChild(span);
...
function pivotamento(){
    ...
    */
    ...
    //Cria matriz operacao (espelho da padrao)
    for(let linha=0; linha<numRestr+2; linha++){ //num linhas -> num restrição + func. obj + CRs
    ...
    console.log("\n\nINFORMACOES SOBRE O PIVOTAMENTO")
...
let linhasQueJaForam = [];

//I. Verifica se há restrições não inteiras
function verificaInteiro(){
    var linhaRacional=0;
    for(linha=1; linha<numInicialRestr+1; linha++){
        for(coluna=1; coluna<numVariaveis+(numInicialRestr+2); coluna++){
            linhaRacional=linha;
            if((matrizPadrao[linha][coluna]%1) !== 0 && (matrizPadrao[linha][coluna]%1) !== -0 && !linhasQueJaForam.includes(linhaRacional)){
                linhasQueJaForam.push(linhaRacional);
                return linhaRacional;
            } 
        }
    }
    
    linhasQueJaForam.length=0;
    for(linha=1; linha<numInicialRestr+1; linha++){
        for(coluna=1; coluna<numVariaveis+(numInicialRestr+2); coluna++){
            linhaRacional=linha;
            if((matrizPadrao[linha][coluna]%1) !== 0 && (matrizPadrao[linha][coluna]%1) !== -0 && !linhasQueJaForam.includes(linhaRacional)){
...
//II. Cria nova restrição inteira
function criaNovaRestricao(linhaNaoInt){
    let novaRestricao = [];
    novaRestricao.push(10000);

    let linhaCR = [];
    for(let coluna=0; coluna<numVariaveis+(numRestr*2); coluna++){
        linhaCR.push(matrizPadrao[numRestr+1][coluna]);
    }
    linhaCR.push(0);
    linhaCR.push(0);
    linhaCR.push(matrizPadrao[numRestr+1][numVariaveis+(numRestr*2)]);
    linhaCR.push(matrizPadrao[numRestr+1][numVariaveis+(numRestr*2)+1]);

    numRestr++;
    console.log("\nEh necessario fazer o corte de Gomory -> Linha que sera \"truncada\": " + linhaNaoInt);

    //Ajuste da matriz padrao para comportar nova restricao
    matrizPadrao[0][numVariaveis+(2*numRestr)-1]=0;
    matrizPadrao[0][numVariaveis+(2*numRestr)]=10000;

    for(let linha=1; linha<numRestr+1; linha++){
        matrizPadrao[linha].length = numVariaveis+(2*numRestr)+2;

        matrizPadrao[linha][numVariaveis+(2*numRestr)+1]=matrizPadrao[linha][numVariaveis+(2*numRestr)-1];
        matrizPadrao[linha][numVariaveis+(2*numRestr)-1]=0;
        matrizPadrao[linha][numVariaveis+(2*numRestr)]=0;
    }
    

    //Criacao da nova restricao
    for(let coluna=1; coluna<numVariaveis+(numRestr*2)-1; coluna++){
        if(matrizPadrao[linhaNaoInt][coluna]<0) novaRestricao.push(Math.ceil(matrizPadrao[linhaNaoInt][coluna]*-1)+matrizPadrao[linhaNaoInt][coluna]);
        else novaRestricao.push(matrizPadrao[linhaNaoInt][coluna]-Math.floor(matrizPadrao[linhaNaoInt][coluna]));
    }

    novaRestricao.push(-1);
    novaRestricao.push(1);
    if(matrizPadrao[linhaNaoInt][numVariaveis+(2*numRestr)+1]<0) novaRestricao.push(Math.ceil(matrizPadrao[linhaNaoInt][numVariaveis+(2*numRestr)+1]*-1)+matrizPadrao[linhaNaoInt][numVariaveis+(2*numRestr)+1]);
    else novaRestricao.push(matrizPadrao[linhaNaoInt][numVariaveis+(2*numRestr)+1]-Math.floor(matrizPadrao[linhaNaoInt][numVariaveis+(2*numRestr)+1]));
   

    for(let coluna=0; coluna<numVariaveis+(numRestr*2)+2; coluna++){
        matrizPadrao[numRestr][coluna]=novaRestricao[coluna];
    }
    //matrizPadrao.push(linhaCR);
    matrizPadrao.splice(numRestr+1, 0, linhaCR);
}
 



//7. IMPRESSÃO DO SIMPLEX ======================================================================================================================
...
function imprimeSimplex(){
    ...
    //I. Resultado (Z)
    console.log("\nResultado: z = " + resultado.toFixed(5));

    const span = document.createElement('h2');
    span.textContent = "Solução Inteira Única";
    divResultado.appendChild(span);
